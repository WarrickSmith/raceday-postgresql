...
async function fetchRacingData(baseUrl, context) {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `NZTAB API request failed: ${response.status} ${response.statusText}`
      );
    }
...
async function processMeetingsAndRaces(
  ...
  for (let i = 0; i < allRaces.length; i += batchSize) {
    ...
    const racePromises = batch.map(async ({ meeting, race }) => {
      try {
        const raceDoc = {
          ...
          ...(race.track_condition !== undefined && {
    ...
    const successfulRaces = batchResults.filter(
