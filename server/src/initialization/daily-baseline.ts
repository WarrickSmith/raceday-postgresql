...
const transformMeeting = (meeting: MeetingData): TransformedMeeting => {
...
const transformRacesForBaseline = (
  ...
  for (const race of racePayloads) {
    if (isMeetingData(race.meeting)) {
      meetingsMap.set(race.meeting.meeting, transformMeeting(race.meeting))
    }
