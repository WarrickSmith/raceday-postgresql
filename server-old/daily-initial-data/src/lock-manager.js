/**
 * Execution lock management for daily-initial-data function
...
const STALE_LOCK_THRESHOLD_MS = 3 * 60 * 1000; // 3 minutes (shorter due to lighter function)
const HEARTBEAT_INTERVAL_MS = 60 * 1000; // 1 minute heartbeat updates

/**
 * Fast lock check - must complete in <50ms to minimize CPU waste
...
export async function fastLockCheck(databases, databaseId, context) {
    ...
    try {
        ...
        // Check if this is a collision (another instance holds the lock)
            ...
            try {
                ...
                if (lockAge > STALE_LOCK_THRESHOLD_MS) {
                    ...
                    logDebug(context, 'Daily-initial-data lock held by active instance - terminating to save resources', {
                        ...
                        resourcesSaved: {
                            noRaceQueryProcessing: true,
                            noBatchExecution: true,
                            cpuTimeSaved: 'prevented expensive batch operations'
                        }
                    });
                    return null;
                }
            } catch (getLockError) {
                context.error('Failed to check existing daily-initial-data lock status', {
            ...
            context.error('Unexpected error during fast daily-initial-data lock acquisition', {
                error: error.message,
                acquisitionTimeMs: acquisitionTime,
                executionId
            });
...
export async function updateHeartbeat(lockManager, progress = {}) {
    ...
    try {
        ...
        lockManager.context.error('Failed to update daily-initial-data lock heartbeat', {
            executionId: lockManager.executionId,
            error: error.message
        });
    }
...
export async function releaseLock(lockManager, completionStats = {}, status = 'completed') {
    ...
    try {
        ...
        lockManager.context.error('Failed to release daily-initial-data execution lock', {
            executionId: lockManager.executionId,
            error: error.message,
            status
        });
...
export function shouldTerminateForNzTime(context) {
    try {
        ...
        context.error('Failed to check NZ termination time for daily-initial-data', {
            error: error.message
        });
