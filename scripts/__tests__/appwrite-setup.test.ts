...
describe('Appwrite Setup Script', () => {
  ...
  describe('setupAppwrite', () => {
    ...
    it('should handle existing resources (idempotent)', async () => {
      // Ensure required environment variables are set before import
      process.env.APPWRITE_PROJECT_ID = 'test-project-id'
      process.env.APPWRITE_API_KEY = 'test-api-key'

      // Mock that resources already exist
      mockDatabases.get.mockResolvedValue({}) // Database exists
      mockDatabases.getCollection.mockImplementation(
        (dbId: string, collectionId: string) => {
          const allAttributes = [
            // meetings
            { key: 'meetingIdentifier', status: 'available' },
            { key: 'name', status: 'available' },
          ...
          const allIndexes = [
            ...
            { key: 'idx_entrant_id' },
            // oddsHistory
            { key: 'idx_entrant_timestamp' },
            // moneyFlowHistory
            { key: 'idx_money_flow_entrant_timestamp' },
            // userAlertConfigs
            { key: 'idx_user_alert_type' },
            // notifications
            { key: 'idx_user_created_at' },
          ]
      ...
      await expect(freshSetupAppwrite()).resolves.toBeUndefined()

      expect(mockLog).toHaveBeenCalledWith(
        'ℹ️ Database already exists, skipping creation'
      )
      // The main point of this test is to ensure that the setup does not hang or crash
      // when resources already exist. The exact log output is less critical.
      // By expecting the promise to resolve, we confirm the idempotent nature of the script.
    }, 20000) // Set timeout to 20 seconds for this test
  })
