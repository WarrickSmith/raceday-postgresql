...
export const RaceNavigation = memo(function RaceNavigation({
  ...
  const navigationStates = useMemo((): {
    ...
    return {
      previous: {
        ...
        disabledReason: !navigationData.previousRace
      ...
      next: {
        ...
        disabledReason: !navigationData.nextRace
          ? 'No more scheduled races'
          : undefined,
      },
      nextScheduled: {
        isLoading: isNavigating,
        disabled: !navigationData.nextScheduledRace || isNavigating,
        disabledReason: !navigationData.nextScheduledRace
  ...
  const handleNavigation = useCallback(
    async (
      direction: 'previous' | 'next' | 'nextScheduled' | 'backToMeetings'
    ) => {
      if (isNavigating) {
      ...
      switch (direction) {
        case 'previous':
          ...
          console.log(
            '⬅️ Navigating to previous race:',
            navigationData.previousRace.raceId
          )
        ...
        case 'next':
          ...
          console.log(
            '➡️ Navigating to next race:',
            navigationData.nextRace.raceId
          )
        ...
        case 'nextScheduled':
          ...
          console.log(
            '⏰ Navigating to next scheduled race:',
            navigationData.nextScheduledRace.raceId
          )
        ...
        promises.push(
          raceCache
            .get(
              ...
              () =>
                fetch(
                  `/api/race/${navigationData.previousRace!.raceId}?nav=true`
                ).then((r) => r.json()),
              15000
            )
            .catch((error) =>
              console.warn('Failed to pre-cache previous race:', error)
            )
        )
      }
        ...
        promises.push(
          raceCache
            .get(
              ...
              () =>
                fetch(
                  `/api/race/${navigationData.nextRace!.raceId}?nav=true`
                ).then((r) => r.json()),
              15000
            )
            .catch((error) =>
              console.warn('Failed to pre-cache next race:', error)
            )
        )
      }
      ...
      if (navigationData.nextScheduledRace) {
        promises.push(
          raceCache
            ...
            .catch((error) =>
              console.warn('Failed to pre-cache next scheduled race:', error)
            )
        )
      }
      ...
      if (promises.length > 0) {
        Promise.all(promises).then(() => {
          console.log(
            '✅ Pre-cached',
            promises.length,
            'adjacent races for instant navigation'
          )
        })
      }
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-describedby={
          navigationStates.next.disabledReason ? 'next-race-desc' : undefined
        }
      >
      ...
      <div className="sr-only" aria-hidden="true">
        Use left and right arrow keys to navigate between buttons. Press Enter
