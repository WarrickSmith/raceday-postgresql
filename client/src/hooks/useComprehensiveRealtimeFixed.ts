...
import {
  Entrant,
  Race,
  EntrantSubscriptionResponse,
  MoneyFlowSubscriptionResponse,
  OddsHistorySubscriptionResponse,
  OddsHistoryData,
} from '@/types/meetings'
...
export function useComprehensiveRealtime({
  ...
  const setupSubscriptions = useCallback(async () => {
    try {
      ...
      subscriptionsRef.current.entrants = client.subscribe(
        channels.entrants,
        (response: EntrantSubscriptionResponse) => {
          if (!response.payload) return

          // Normalize race reference (Appwrite may return either a string id or a document reference object)
          const payloadRaceRef = (response.payload as any).race
          const payloadRaceId =
            typeof payloadRaceRef === 'string'
              ? payloadRaceRef
              : (payloadRaceRef && (payloadRaceRef as { $id?: string }).$id) ||
                String(payloadRaceRef)

          if (payloadRaceId !== raceId) return

          const eventType = response.events?.[0] || ''

          // Handle create/update/delete events robustly
          if (eventType.includes('create') && response.payload) {
            setEntrants((current) => {
              const created = response.payload as Entrant
              // Avoid duplicates
              const exists = current.some((e) => e.$id === created.$id)
              return exists
                ? current
          ...
          } else if (eventType.includes('update') && response.payload) {
            setEntrants((current) =>
              current.map((entrant) =>
          ...
          } else if (eventType.includes('delete') && response.payload) {
            setEntrants((current) =>
              current.filter((e) => e.$id !== response.payload!.$id)
            )
          }
      ...
      subscriptionsRef.current.moneyFlow = client.subscribe(
        channels.moneyFlow,
        (response: MoneyFlowSubscriptionResponse) => {
          ...
          const entrantId =
            typeof entrantRef === 'string'
              ? entrantRef
              : (entrantRef as { $id?: string })?.$id || String(entrantRef)
          ...
          setEntrants((current) => {
            console.log(
              '[DEBUG] Current entrants:',
              current.map((e) => ({ id: e.$id, name: e.name }))
            )
            ...
            const updated = current.map((entrant) => {
              if (entrant.$id === entrantId && holdPercentage !== undefined) {
                ...
                if (
                  ...
                  trend =
                    holdPercentage > entrant.holdPercentage ? 'up' : 'down'
                }
            ...
            console.log(
              '[DEBUG] Updated entrants with money flow:',
              updated.map((e) => ({
                id: e.$id,
                name: e.name,
      ...
      subscriptionsRef.current.oddsHistory = client.subscribe(
        channels.oddsHistory,
        (response: OddsHistorySubscriptionResponse) => {
          if (!response.payload || !response.payload.entrant) return
          ...
          setUpdateCounts((prev) => ({
            ...prev,
            oddsHistory: prev.oddsHistory + 1,
          }))
        }
      )
      subscriptionCount++
