...
**Status**: Needs Architecture Update
...
- [ ] T2: Add last_poll_time field to races collection for polling coordination (AC: 1,3)
  - [ ] ST2.1: Add `last_poll_time` datetime field to races collection schema
  - [ ] ST2.2: Update existing race documents to initialize `last_poll_time` field
  - [ ] ST2.3: Modify all polling functions to update `last_poll_time` when processing races
  - [ ] ST2.4: Add database indexes for efficient querying by `last_poll_time`
