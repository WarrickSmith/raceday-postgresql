...
- [ ] T2: Create enhanced type definitions and interfaces (AC: 1,2,3,4)
  - [ ] ST2.1: Create `/src/types/moneyFlow.ts` with money flow timeline data structures
  - [ ] ST2.2: Create `/src/types/racePools.ts` with pool totals and race results interfaces
  - [ ] ST2.3: Create `/src/types/jockeySilks.ts` with silk color and pattern data structures
  - [ ] ST2.4: Extend existing race data types to include enhanced v4.7 data
  - [ ] ST2.5: Create enhanced grid state management interfaces for sorting and pool toggle
...
- [ ] T4: Create MoneyFlowColumns component with horizontal scrolling (AC: 2,4,6)
  - [ ] ST4.1: Create `/src/components/race-view/MoneyFlowColumns.tsx` with virtualized horizontal scrolling
  - [ ] ST4.2: Implement time column generation (T-60m to Start) based on polling intervals
  - [ ] ST4.3: Add incremental money flow calculation and display logic
  - [ ] ST4.4: Implement visual indicators (⚡ lightning, color coding) for money flow changes
  - [ ] ST4.5: Add sticky header and runner name column for horizontal scroll navigation
  - [ ] ST4.6: Optimize performance with React.memo, useMemo for column generation

- [ ] T5: Create enhanced EnhancedEntrantsGrid component (AC: 2,3,4,6)
  - [ ] ST5.1: Create `/src/components/race-view/EnhancedEntrantsGrid.tsx` with professional layout
  - [ ] ST5.2: Integrate MoneyFlowColumns component with main entrants data
  - [ ] ST5.3: Implement virtualization for large datasets and smooth scrolling performance
  - [ ] ST5.4: Add enhanced runner information display with jockey silks integration
  - [ ] ST5.5: Implement responsive design with mobile-first approach and container queries
  - [ ] ST5.6: Add sortable column headers with visual indicators and keyboard support

- [ ] T6: Implement PoolToggle and SortableColumns functionality (AC: 4,5)
  - [ ] ST6.1: Create `/src/components/race-view/PoolToggle.tsx` for Win/Place/Odds view switching
  - [ ] ST6.2: Create `/src/components/race-view/SortableColumns.tsx` with interactive sorting
  - [ ] ST6.3: Implement keyboard navigation support (Space, Arrow keys, Tab)
  - [ ] ST6.4: Add sort state persistence per session with local storage
  - [ ] ST6.5: Implement ARIA labels and accessibility features for interactive elements
  - [ ] ST6.6: Add visual feedback for user interactions and state changes

- [ ] T7: Create JockeySilks and enhanced visual components (AC: 2,3)
  - [ ] ST7.1: Create `/src/components/race-view/JockeySilks.tsx` for color-coded silk display
  - [ ] ST7.2: Implement silk pattern rendering with CSS and SVG graphics
  - [ ] ST7.3: Add fallback handling for missing or incomplete silk data
  - [ ] ST7.4: Implement efficient caching strategy for silk images and patterns
  - [ ] ST7.5: Add high contrast and accessibility-compliant color alternatives

- [ ] T8: Enhance RaceHeader and create RaceFooter components (AC: 1,3)
  ...
  - [ ] ST8.3: Create new `/src/components/race-view/RaceFooter.tsx` with comprehensive race status
  - [ ] ST8.4: Implement race results display and pool breakdown information
  - [ ] ST8.5: Add responsive design for various screen sizes and orientations
